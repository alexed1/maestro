({
  ...
  updateLogstream : function(cmp, eventData) {
      ...
      console.log('creating component: ' + eventData.Label__c );
    $A.createComponent(
        "c:logLine",
        {"recordId" : eventData.Record_ID__c,
         "recordTypeName": eventData.Record_Name__c ,
         "message" : eventData.Label__c 
        },
            
            function(newCmp) {
                if(cmp.isValid()){
                    console.log('successfully created component: ' +newCmp );
					var body = cmp.get("v.body");
                    body.unshift(newCmp);
                    cmp.set("v.body", body);  
                }
